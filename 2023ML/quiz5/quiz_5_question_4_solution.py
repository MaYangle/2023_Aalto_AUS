...
class AdaBoost_cls:
    ...
    def fit(self, X, y):
        ...
        ## implement the AdaBoost algorithm on the learners given in self.llearners
            ...
            y_pred = self.llearners[t].predict(X)
            theta_t = np.min(np.sum(self.D * (y_pred != y)))

            self.alpha[t] = 1/2 * np.log((1 - theta_t)/theta_t)

            Z_t = 2*np.sqrt(theta_t * (1-theta_t))
            self.D = self.D * np.exp(-self.alpha[t] * y * y_pred)
            self.D /= Z_t
    ...
    def predict(self, Xtest):
        ...
        for t in range(self.T):
            ypred = self.llearners[t].predict(Xtest)
            ypredict += self.alpha[t] * ypred
...
def main(iworkmode):
    ...
    for ds_cnt, ds in enumerate(datasets):
        ...
        best_learner_index = np.argmax(adaboost.alpha)
        best_learner_name = learner_names[best_learner_index]
    ...
    ##   and the learners with the highest alpha
        print(f'Dataset: {data_names[ds_cnt]}, F1 Score: {f1:.4f}, Best Learner: {best_learner_name}')
    print('Bye')
